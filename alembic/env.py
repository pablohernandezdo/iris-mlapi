...
from sqlalchemy import create_engine, engine_from_config, make_url, pool, text
...
def ensure_database_exists(url: str) -> None:
    ...
    Connects to the 'master' database (SQL Server system DB) and issues a
    CREATE DATABASE only when the target database is missing.  Uses
...
ensure_database_exists(settings.db_url.get_secret_value())
